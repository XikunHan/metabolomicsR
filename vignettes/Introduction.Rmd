...
<strong>Figure:</strong> Seamless workflow to preprocess, analyze, and visualize metabolomic data in `{metabolomicsR}` <img src="../man/figures/workflow.png" align="center" width="120%" height="180%"/>
...
The `{update_Metabolite}` fuction provides several methods to update the data, including "injection_order", "keep_feature", "remove_feature", "keep_sample", "remove_sample", "add_sample_annotation", and "change_featureID". 
...
Normalization of metabolomic data is an important step to remove systematic variation, and to facilitate the identification of real biological metabolites. Here we provided the following normalization methods: batch-based normalization, QC sample-based/nearest QC sample-based normalization, and LOESS normalization. The last three methods are used when the internal QC samples are available.
